...
export function CampaignTabsList({ campaign }: CampaignTabsProps) {
  return (
    <TabsList className="mb-8 h-auto w-fit justify-start rounded-none border-b border-brand-light-gray/20 bg-transparent p-0">
      ...
      {(campaign.status === 'SELESAI' || campaign.status === 'Selesai') && (
        <TabsTrigger
          value="energy"
          className="flex items-center gap-2 rounded-none border-b-2 border-transparent px-4 py-3 text-brand-light-gray data-[state=active]:border-brand-green data-[state=active]:bg-transparent data-[state=active]:text-brand-green data-[state=active]:shadow-none"
        >
...
export function CampaignTabsContent({ campaign, expenses = [] }: CampaignTabsProps) {
  // Calculate financial summary
  const totalDisbursed = expenses.reduce((sum, expense) => sum + expense.amountIdr, 0);
                ...
                {expenses.length > 0 ? (
                  <>
                    {expenses.map((expense) => (
                      <TableRow key={expense.id} className="border-none hover:bg-transparent">
                        <TableCell className="py-4 text-brand-gray">{formatDate(expense.spentAt)}</TableCell>
                        <TableCell className="py-4 text-brand-gray">{formatCurrency(expense.amountIdr)}</TableCell>
                    ...
                    <TableRow className="border-none hover:bg-transparent">
                      ...
                      <TableCell className="py-3 text-base font-bold text-brand-text">{formatCurrency(remainingFunds)}</TableCell>
                      <TableCell className="py-3"></TableCell>
                    </TableRow>
                  </>
                ) : (
                  <TableRow className="border-none hover:bg-transparent">
                    <TableCell colSpan={3} className="py-8 text-center text-brand-gray">
