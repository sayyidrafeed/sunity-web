...
export function KelolaKampanyePage() {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      case 'Aktif': return 'bg-blue-50 text-blue-600';
      case 'Instalasi': return 'bg-orange-50 text-orange-600';
          ...
          {['All', 'Aktif', 'Instalasi', 'Selesai'].map(tab => (
            <button 
              ...
              className={`pb-2 transition ${
                activeTab === tab 
                  ? 'border-b-2 border-[#2F6B3F] text-[#2F6B3F] font-bold' 
                  : 'text-gray-500 hover:text-[#2F6B3F]'
              }`}
            >
            ...
            <div key={c.id} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex gap-6 hover:shadow-md transition">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  ...
                  <span className={`px-4 py-1 rounded-full text-sm font-medium ${getStatusColor(c.status)}`}>
                    {c.status}
                  </span>
                </div>
                ...
                <div className="w-full bg-gray-200 h-2 rounded-full mt-2 mb-4">
                  <div 
                    className="bg-[#2F6B3F] h-2 rounded-full" 
