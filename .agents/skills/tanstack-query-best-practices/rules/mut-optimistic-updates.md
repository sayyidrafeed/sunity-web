...
const mutation = useMutation({
  ...
  onMutate: async (todoId) => {
    ...
    queryClient.setQueryData(['todos'], (old: Todo[]) => old.map((todo) => (todo.id === todoId ? { ...todo, completed: !todo.completed } : todo)));
...
function TodoItem({ todo }: { todo: Todo }) {
  ...
  return (
    <div>
      <input type="checkbox" checked={displayCompleted} disabled={mutation.isPending} onChange={() => mutation.mutate(todo.id)} />
...
const createTodo = useMutation({
  ...
  onSuccess: (data, variables, context) => {
    ...
    queryClient.setQueryData(['todos'], (old: Todo[]) => old.map((todo) => (todo.id === context?.optimisticTodo.id ? data : todo)));
  },
});
```
